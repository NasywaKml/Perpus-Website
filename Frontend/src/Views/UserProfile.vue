...
async function loadBorrowingHistory(idUser) {
  try {
    const res = await axios.get(`${API_BASE}/api/peminjaman/user/${idUser}`);
    ...
    const enriched = await Promise.all(history.map(async (item) => {
      ...
      try {
        if (item.idBuku) {
          const bookRes = await axios.get(`${API_BASE}/api/buku/${item.idBuku}`);
          if (bookRes.ok || bookRes.status === 200) {
            const bookData = bookRes.data || {};
            image = bookData.urlGambarSampul || null;
            author = bookData.pengarang || author;
          }
        }
      } catch (err) {
        // ignore per-item failure and fallback to availableBooks
        console.warn("Failed to fetch book detail for id", item.idBuku, err);
      }

      // fallback: try to find by title in availableBooks
      if (!image) {
...
async function fetchProfile() {
  try {
    ...
    // Load and enrich borrowing history
