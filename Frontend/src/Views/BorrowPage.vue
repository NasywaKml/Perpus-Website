            ...
            <div class="modal-details">
              ...
              <div class="payment-box">
                <div class="payment-row">
                  ...
                  <span class="muted">IDR 70.000,00</span>
                </div>
                <div class="payment-row">
                  <span>Total Amount</span>
                  <span class="bold">IDR 70.000,00</span>
                </div>
              </div>
            </div>
...
const mainTab = ref(route.query.tab === 'borrow' ? 'borrow' : 'history');
...
async function loadAvailableBooks() {
  try {
    ...
    if (books.length > 0) {
      console.log("First book object keys:", Object.keys(books[0]));
      console.log("First book object:", books[0]);
    }
    
    // Transform API response ke format yang sesuai dengan UI
    availableBooks.value = books.map(book => {
      console.log("Mapping book - idBuku:", book.idBuku, "judul:", book.judul);
      return {
        id: book.idBuku,
        title: book.judul,
        author: book.pengarang,
        image: book.urlGambarSampul,
        borrowed: book.jumlahStok <= 0,
        fee: book.hargaSewa,
    ...
    if (availableBooks.value.length > 0) {
      console.log("First transformed book:", availableBooks.value[0]);
    }
  } catch (err) {
...
/* ============================
   HELPERS
============================ */
// Format date to MM/DD/YYYY as per design images
...
const extendedDueDate = computed(() => {
  ...
  return current.toLocaleDateString("en-US", { month: 'numeric', day: 'numeric', year: 'numeric' });
});
...
async function loadBorrowingHistory(idUser) {
    try {
        const res = await fetch(`http://localhost:8080/api/peminjaman/user/${idUser}`);
    ...
    managedBooks.value = await Promise.all(history.map(async (item) => {
      ...
      try {
        if (item.idBuku) {
          const bookRes = await fetch(`http://localhost:8080/api/buku/${item.idBuku}`);
          if (bookRes.ok) {
            const bookData = await bookRes.json();
            bookImage = bookData.urlGambarSampul || bookImage;
            bookFee = bookData.hargaSewa || 0;
            bookAuthor = bookData.pengarang || bookAuthor;
          }
        } else {
          // Fallback: cari di availableBooks berdasarkan judul
...
function openBorrowModal(book) {
  ...
  // Langsung assign book object, karena sudah di-transform dengan benar
...
async function finishPayment() { 
  ...
  try {
    ...
    console.log("DEBUG finishPayment - selectedBook.value?.id:", selectedBook.value?.id);
    
    if (!selectedBook.value?.id) {
      alert("Book ID is not set. Please select a book again.");
      return;
    }
    
    console.log("DEBUG: Submitting borrow request with:");
    console.log("  - idPemustaka:", parseInt(idUser));
    console.log("  - idBuku:", selectedBook.value.id);
    console.log("  - tanggalPinjam:", startDate.value);
    
    const peminjamanRequest = {
      idPemustaka: parseInt(idUser),
      idBuku: selectedBook.value.id,
    ...
    if (!res.ok) {
      const errorMsg = responseData.error || "Failed to borrow book";
...
function closeModal() {
  ...
  const idUser = sessionStorage.getItem("idUser");
  if (idUser) {
    loadBorrowingHistory(idUser);
  }
}

onMounted(async () => {
  const idUser = await loadUserDataFromSession();
  await loadBooks();
  
  if (idUser) {
    await loadBorrowingHistory(idUser);
  }
});
</script>
